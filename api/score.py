...
from schemas.score import (
    ScoreCreate, ScoreUpdate, ScoreResponse, ScoreResponseItem, ScorePageResponse,
...
@router.get('', response_model=ScorePageResponse, summary="综合查询成绩")
def get_scores(
...
@router.post("/restore", summary="批量/单条恢复已删除成绩")
def restore_score(
    id: Optional[int] = Query(None, description="成绩ID", ge=1),
    student_no: Optional[str] = Query(None, description="学生学号", min_length=1, max_length=20),
    ...
    参数说明：
    - id: 按成绩ID恢复
    - student_no: 按学号恢复该学生所有已删除成绩
    - exam_order: 按考试序号恢复该次考试所有已删除成绩
    - 至少提供一个参数，支持组合条件恢复
    """
    if id is None and student_no is None and exam_order is None:
        raise HTTPException(status_code=400, detail="至少提供一个查询条件")
