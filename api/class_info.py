...
def get_all_classinfo(db:Session = Depends(get_db), _: AuthUser = Depends(require_login)) -> Dict[str, Any]:
    """获取所有班级信息列表"""
...
def get_one_classinfo(class_id: int,db: Session = Depends(get_db), _: AuthUser = Depends(require_login)) -> Dict[str, Any]:
    """根据班级ID查询单个班级信息"""
...
@router.post("/restore/{class_id}", summary="恢复逻辑删除的班级", response_model=dict)
...
def count_class_month(month: str = None, db: Session = Depends(get_db), _: AuthUser = Depends(require_login)) -> Dict[str, Any]:
    """统计每月开班数量，支持按月份筛选"""
...
def get_class_by_lecturer_id(lecturer_id: int,db: Session = Depends(get_db), _: AuthUser = Depends(require_login)) -> Dict[str, Any]:
    """根据讲师ID查询其负责的班级列表"""
